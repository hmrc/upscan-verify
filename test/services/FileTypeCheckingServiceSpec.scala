...
class FileTypeCheckingServiceSpec extends UnitSpec with GivenWhenThen with WithIncrementingClock {
  ...
  "FileTypeCheckingService" should {
    "return valid result for allowed mime type for the service" in {
      ...
      val detector = new MimeTypeDetector {
        override def detect(inputStream: InputStream, fileName: Option[String]) =
          Right(MimeType("application/pdf"))
      }
    ...
    "return invalid result when mime type detector returns error" in {
      ...
      val location = S3ObjectLocation("inbound-bucket", "valid-file", None)
      val content  = ObjectContent(null, 1200)
      val metadata = InboundObjectMetadata(Map("consuming-service" -> "valid-test-service", "original-filename" -> "file.foo"), Instant.now, 0)

      val detector = new MimeTypeDetector {
        override def detect(inputStream: InputStream, fileName: Option[String]) =
          Left(NotAllowedFileExtension(MimeType("application/pdf"), "foo"))
      }
